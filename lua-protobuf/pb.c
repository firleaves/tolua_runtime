...
static uint64_t lpb_tointegerx(lua_State *L, int idx, int *isint) {
	...
	//检查是否是userdata
	if (tolua_isuint64(L, idx))
	{
		uint64_t v = tolua_touint64(L, idx);
		*isint = 1;
		return v;
	}
	if (tolua_isint64(L, idx))
	{
		uint64_t v = (uint64_t)tolua_toint64(L, idx);
		*isint = 1;
		return v;
	}
...
#define pushinteger(n) lpb_pushinteger((L), (n), LS->int64_mode)
    ...
    case PB_Tint64: case PB_Tuint64: case PB_Tsint64:
        ...
        //改用userdata处理
        case PB_Tint64:
            tolua_pushint64(L, (int64_t)v.u64);
            break;
        case PB_Tuint64:
            tolua_pushuint64(L, (int64_t)v.u64);
            break;
        case PB_Tsint64:
            tolua_pushint64(L, pb_decode_sint64(v.u64));
            break;
        }
        break;
    case PB_Tfloat:
